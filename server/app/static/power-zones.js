...
function powerZonePreview(workout,ftp,large=false) {
  ...
  const power=[],cadence=[];let elapsed=0;
  for(const block of workout.blocks||[]) {
    ...
    if(block.target_cadence!=null)cadence.push([elapsed,block.target_cadence],[elapsed+duration,block.target_cadence]);
    elapsed+=duration;
  }
  ...
  if(cadence.length){
    svg.append(svgElement('polyline',{points:cadence.map(p=>x(p[0])+','+cy(p[1])).join(' '),fill:'none',stroke:'#275d9b','stroke-width':2,'stroke-dasharray':'5 4'}));
